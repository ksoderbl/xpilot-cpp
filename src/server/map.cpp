...
{
    cannon_t *c = Cannon_by_index(world->NumCannons);
    c->dir = dir;
    c->blk_pos = Clpos_to_blkpos(pos);
    c->pix_pos = Clpos_to_position(pos);
    c->pos = pos;
    c->dead_time = 0;
    c->conn_mask = (unsigned)-1;
    c->team = team;
    Cannon_init(c);
    return -1;
}
        ...
        for (x = 0; x < world->x; x++)
        {
            ...
            for (y = 0; y < world->y; y++)
            {
                ...
                case '9':
                    ...
                    world->bases[world->NumBases].ind = world->NumBases;
                    world->NumBases++;
                    break;
