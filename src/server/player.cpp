...
void Go_home(Player *pl)
{
    ...
    debugprint("===> Go_home: player %s, ind = %d, pl->ind = %d", pl->name.c_str(), ind, pl->ind);
    ...
    if (ind != pl->ind)
    {
        ...
        debugprint("******** ERROR!!!!");
        debugprint("pl1: '%s', pl2: '%s'", pl1->name.c_str(), pl2->name.c_str());
    }
...
void Player_death_reset(Player *pl, bool add_rank_death)
{
    ...
    debugprint("before: player %s, pl->obj_status = 0x%08x, s = 0x%08x (%s / %s)",
    ...
    debugprint("after : player %s, pl->obj_status = 0x%08x, s = 0x%08x (%s / %s)",
               pl->name.c_str(), pl->obj_status, s, bitsToStr(pl->obj_status).c_str(), bitsToStr(s).c_str());
    ...
    debugprint("after2: player %s, pl->obj_status = 0x%08x, s = 0x%08x (%s / %s)",
               pl->name.c_str(), pl->obj_status, s, bitsToStr(pl->obj_status).c_str(), bitsToStr(s).c_str());
