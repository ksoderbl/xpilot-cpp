...
{
    bool newWaiting = (pl->pl_state == PL_STATE_WAITING);
    bool legacyWaiting = (BIT(pl->obj_status, LEGACY_GAME_OVER) && pl->mychar == 'W');

    if (newWaiting != legacyWaiting)
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_waiting: newWaiting != legacyWaiting: Player %s (%d, %d)", pl->name.c_str(), newWaiting, legacyWaiting);
    }

    return legacyWaiting;

    // if (BIT(pl->obj_status, LEGACY_GAME_OVER) && pl->mychar == 'W')
    //     return true;
    // return false;
    // // TODO
...
// TODO: Rename this to Player_is_recovering ?
static inline bool Player_is_appearing(player_t *pl)
{
    ...
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_appearing: newAppearing != legacyAppearing: Player %s (%d, %d)", pl->name.c_str(), newAppearing, legacyAppearing);
    }
...
{
    bool newAlive = (pl->pl_state == PL_STATE_ALIVE);
    bool legacyAlive = (BIT(pl->obj_status, LEGACY_PLAYING | LEGACY_PAUSE | LEGACY_GAME_OVER | LEGACY_KILLED) == LEGACY_PLAYING);

    if (newAlive != legacyAlive)
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_alive: newAlive != legacyAlive: Player %s (%d, %d)", pl->name.c_str(), newAlive, legacyAlive);
    }

    return legacyAlive;

    // if (BIT(pl->obj_status, LEGACY_PLAYING | LEGACY_PAUSE | LEGACY_GAME_OVER | KILLED) == LEGACY_PLAYING)
    //     return true;
    // return false;
}
...
{
    bool newKilled = (pl->pl_state == PL_STATE_KILLED);
    bool legacyKilled = BIT(pl->obj_status, LEGACY_KILLED);

    if (newKilled != legacyKilled)
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_killed: newKilled != legacyKilled: Player %s (%d, %d)", pl->name.c_str(), newKilled, legacyKilled);
    }
...
{
    bool newDead = (pl->pl_state == PL_STATE_DEAD);
    bool legacyDead = BIT(pl->obj_status, LEGACY_GAME_OVER);

    if (newDead != legacyDead)
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_dead: newDead != legacyDead: Player %s (%d, %d)", pl->name.c_str(), newDead, legacyDead);
    }

    return legacyDead;

    // return pl->pl_state == PL_STATE_DEAD ? true : false;
    // if (BIT(pl->obj_status, LEGACY_GAME_OVER))
    //     return true;
    // return false;
}
...
{
    bool newPaused = (pl->pl_state == PL_STATE_PAUSED);
    bool legacyPaused = BIT(pl->obj_status, LEGACY_PAUSE);

    if (newPaused != legacyPaused)
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_paused: newPaused != legacyPaused: Player %s (%d, %d)", pl->name.c_str(), newPaused, legacyPaused);
    }
...
static inline bool Player_is_active(player_t *pl)
{
    ...
    {
        // if ((main_loops % 12) == 0)
        // warn("Player_is_active: newActive != legacyActive: Player %s (%d, %d)", pl->name.c_str(), newActive, legacyActive);
    }
