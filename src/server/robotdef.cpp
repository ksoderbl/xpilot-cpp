...
static bool Really_empty_space(int ind, int x, int y)
{
    ...
    case WORMHOLE:
        if (!wormholeVisible || World.wormHoles[World.itemID[x][y]].type == WORM_OUT)
        {
            return true;
        }
        else
    ...
    case TARGET:
        if (!targetTeamCollision && BIT(World.rules->mode, TEAM_PLAY) && World.targets[World.itemID[x][y]].team == pl->team)
        {
            return true;
        }
        else
    ...
    case CANNON:
        if (teamImmunity && BIT(World.rules->mode, TEAM_PLAY) && World.cannon[World.itemID[x][y]].team == pl->team)
        {
            return true;
        }
        else
...
static int Rank_item_value(int ind, Item_t itemtype)
{
    ...
    if ((IsDefensiveItem(itemtype) && CountDefensiveItems(pl) >= maxDefensiveItems) || (IsOffensiveItem(itemtype) && CountOffensiveItems(pl) >= maxOffensiveItems))
                                             ...
                                             int *mine_i, int *mine_dist)
{
    ...
    for (j = 0; j < obj_count; j++)
    {
        ...
        if (BIT(shot->type, OBJ_TORPEDO | OBJ_SMART_SHOT | OBJ_ASTEROID | OBJ_HEAT_SHOT | OBJ_BALL | OBJ_CANNON_SHOT) || (BIT(shot->type, OBJ_SHOT) && !BIT(World.rules->mode, TIMING) && shot->id != pl->id && shot->id != NO_ID) || (BIT(shot->type, OBJ_MINE) && shot->id != pl->id) || (BIT(shot->type, OBJ_WRECKAGE) && !BIT(World.rules->mode, TIMING)))
        {
            ...
            if ((dx = ((shot->pos.x - pl->pos.x) + (shot->vel.x - pl->vel.x)),
                 dx = WRAP_DX(dx), ABS(dx)) < *mine_dist &&
                (dy = ((shot->pos.y - pl->pos.y) + (shot->vel.y - pl->vel.y)),
                 dy = WRAP_DY(dy), ABS(dy)) < *mine_dist &&
                (distance = LENGTH(dx, dy)) < *mine_dist)
            {
                *mine_i = j;
        ...
        {
            int delta_dir = 0;
            if (*mine_dist > (WIRE_PTR(shot)->size == 1 ? 2 : 4) * BLOCK_SZ && *mine_dist < 8 * BLOCK_SZ && (delta_dir = (pl->dir - Wrap_findDir(shot->pos.x - pl->pos.x, shot->pos.y - pl->pos.y)) < WIRE_PTR(shot)->size * (RES / 10) || delta_dir > RES - WIRE_PTR(shot)->size * (RES / 10)))
            {
    ...
    if (BIT(pl->used, HAS_SHIELD) == 0 && BIT(pl->have, HAS_SHIELD) != 0)
    {
        ...
        for (j = 0; j < NumPulses; j++)
        {
            ...
            if (Team_immune(pulse->id, pl->id))
                continue;
            if (pl->id == pulse->id && selfImmunity)
                continue;
...
static void Robot_default_play(int ind)
{
    ...
    if (!evade_checked)
    {
        ...
        {
            if (playerShielding == 0 && playerStartsShielded != 0 && BIT(pl->have, HAS_SHIELD))
            {
                SET_BIT(pl->used, HAS_SHIELD);
                if (!cloakedShield)
                    CLR_BIT(pl->used, HAS_CLOAKING_DEVICE);
