...
static bool Check_robot_target(player_t *pl, int item_x, int item_y, int new_mode)
{
    ...
    if (delta_dir >= RES / 4 && delta_dir <= 3 * RES / 4)
    {
        ...
        {
            Choose_weapon_modifier(pl, OBJTYPE_MINE);
            if (BIT(world->rules->mode, TIMING))
                Place_mine(pl);
            else
    ...
    if (new_mode == RM_ATTACK || (BIT(world->rules->mode, TIMING) && new_mode == RM_NAVIGATE))
    {
        ...
        else if ((my_data->robot_count % 10) < pl->item[ITEM_MISSILE] && !WITHIN(my_data->robot_count, my_data->last_fired_missile, 10))
        {
            ...
            if (Detect_ship(pl, Player_by_id(pl->lock.pl_id)) && !pl->visibility[GetInd(pl->lock.pl_id)].canSee)
                type = OBJTYPE_HEAT_SHOT;
            if (type == OBJTYPE_SMART_SHOT && !options.allowSmartMissiles)
                type = OBJTYPE_HEAT_SHOT;
            Choose_weapon_modifier(pl, type);
            Fire_shot(pl, type, pl->dir);
            if (type == OBJTYPE_HEAT_SHOT)
                Thrust(pl, false);
                 ...
                 /*&& BIT(my_data->robot_lock, LOCK_PLAYER)*/)
        {
            ...
            {
                Choose_weapon_modifier(pl, OBJTYPE_SMART_SHOT);
                Fire_shot(pl, OBJTYPE_SMART_SHOT, pl->dir);
                my_data->last_fired_missile = my_data->robot_count;
            }
            else
            {
                ...
                {
                    Choose_weapon_modifier(pl, OBJTYPE_SHOT);
...
static bool Ball_handler(player_t *pl)
{
    ...
    if (BIT(pl->have, HAS_BALL) || pl->ball)
    {
        ...
        else
        {
            for (i = 0; i < NumObjs; i++)
            {
                if (Obj[i]->objtype == OBJTYPE_BALL && Obj[i]->id == pl->id)
                {
                    ball = BALL_PTR(Obj[i]);
                    break;
                }
        ...
        tdir = (int)Wrap_findDir((world->treasures[closest_t].blk_pos.bx + 0.5) * BLOCK_SZ - ball->pix_pos.x,
                                 (world->treasures[closest_t].blk_pos.by + 0.5) * BLOCK_SZ - ball->pix_pos.y);
        xdist = (world->treasures[closest_t].blk_pos.bx) - OBJ_X_IN_BLOCKS(ball);
        ydist = (world->treasures[closest_t].blk_pos.by) - OBJ_Y_IN_BLOCKS(ball);
        for (dist = 0;
        ...
        else
        {
            ...
            return (Check_robot_target(pl,
                                       ((int)(world->treasures[closest_t].blk_pos.bx + 0.5) * BLOCK_SZ),
                                       ((int)(world->treasures[closest_t].blk_pos.by + 0.5) * BLOCK_SZ),
                                       RM_NAVIGATE));
        }
    }
    else
    {
        ...
        if (closest_ball == -1 && closest_nt_dist < (my_data->robot_count / 10) * BLOCK_SZ)
        {
            SET_BIT(my_data->longterm_mode, FETCH_TREASURE);
            return (Check_robot_target(pl,
                                       ((int)(world->treasures[closest_nt].blk_pos.bx + 0.5) * BLOCK_SZ),
                                       ((int)(world->treasures[closest_nt].blk_pos.by + 0.5) * BLOCK_SZ),
                                       RM_NAVIGATE));
        }
        else if (closest_ball_dist < (my_data->robot_count / 10) * BLOCK_SZ && closest_ball_dist > options.ballConnectorLength)
        {
            SET_BIT(my_data->longterm_mode, FETCH_TREASURE);
            return (Check_robot_target(pl, Obj[closest_ball]->pix_pos.x,
...
static int Robot_default_play_check_map(player_t *pl)
{
    ...
    if (target_i >= 0)
    {
        dx = ((world->targets[target_i].blk_pos.bx + 0.5) * BLOCK_SZ);
        dy = ((world->targets[target_i].blk_pos.by + 0.5) * BLOCK_SZ);
                                             ...
                                             int *mine_i, int *mine_dist)
{
    ...
    for (j = 0; j < obj_count; j++)
    {
        ...
        if (BIT(OBJ_TYPEBIT(shot->objtype), OBJ_DEBRIS_BIT | OBJ_SPARK_BIT))
            continue;

        dx = WRAP_DX(shot->pix_pos.x - pl->pix_pos.x);
        dy = WRAP_DY(shot->pix_pos.y - pl->pix_pos.y);

        if (shot->objtype == OBJTYPE_BALL && !WITHIN(my_data->last_thrown_ball,
                                                     my_data->robot_count,
                                                     3 * FPS))
            SET_BIT(pl->used, USES_CONNECTOR);

        /* Ignore shots if shields already up - nothing else to do anyway */
        if (BIT(OBJ_TYPEBIT(shot->objtype), OBJ_SHOT_BIT | OBJ_CANNON_SHOT_BIT) && BIT(pl->used, HAS_SHIELD))
            continue;
         ...
         */
        if (!BIT(OBJ_TYPEBIT(shot->objtype), killing_shots))
        {
            /* Find closest item */
            if (shot->objtype == OBJTYPE_ITEM)
            {
                object_t *item = shot;
        ...
        if (BIT(OBJ_TYPEBIT(shot->objtype), OBJ_TORPEDO_BIT | OBJ_SMART_SHOT_BIT | OBJ_ASTEROID_BIT | OBJ_HEAT_SHOT_BIT | OBJ_BALL_BIT | OBJ_CANNON_SHOT_BIT) || (shot->objtype == OBJTYPE_SHOT && !BIT(world->rules->mode, TIMING) && shot->id != pl->id && shot->id != NO_ID) || (shot->objtype == OBJTYPE_MINE && shot->id != pl->id) || (shot->objtype == OBJTYPE_WRECKAGE && !BIT(world->rules->mode, TIMING)))
        {
            ...
            sqr(dx) + sqr(dy) <= sqr(shield_range) && (int)(rfrac() * 100) < (85 + (my_data->defense / 7) - (my_data->attack / 50)))
        {
            ...
            if (BIT(OBJ_TYPEBIT(shot->objtype), OBJ_TORPEDO_BIT | OBJ_SMART_SHOT_BIT | OBJ_ASTEROID_BIT | OBJ_HEAT_SHOT_BIT | OBJ_MINE_BIT) && (pl->fuel.sum < pl->fuel.l3 || !BIT(pl->have, HAS_SHIELD)))
            {
        ...
        if (shot->objtype, OBJTYPE_HEAT_SHOT)
        {
            Thrust(pl, false);
        ...
        if (shot->objtype == OBJTYPE_ASTEROID)
        {
            int delta_dir = 0;
...
static void Robot_default_play(player_t *pl)
{
    ...
    /* KK: unfortunately, this introduced a new bug. robots with large
        ...
        fix it */
    if (QUICK_LENGTH(pl->pix_pos.x - (pl->home_base->blk_pos.bx * BLOCK_SZ),
                     pl->pix_pos.y - (pl->home_base->blk_pos.by * BLOCK_SZ)) < BLOCK_SZ)
    {
        Thrust(pl, true);
    }
        ...
        Detect_ship(pl, locked_pl))
    {
        ...
        if (Check_robot_target(pl, dx, dy, RM_ATTACK) && !BIT(my_data->longterm_mode, FETCH_TREASURE | TARGET_KILL | NEED_FUEL))
            return;
    }
