 ...
 * Private functions.
 */
/*static bool Check_robot_navigate(Player *pl, bool * num_evade);*/
...
static void Robot_default_create(Player *pl, char *str)
{
    ...
    my_data->fuel_l1 = 100.0;
    my_data->fuel_l2 = 200.0;
    my_data->fuel_l3 = 500.0;
...
static void Robotdef_do_tractor_beam(Player *pl)
{
    ...
    if (BIT(pl->lock.tagged, LOCK_PLAYER) &&
        ...
        pl->lock.distance < TRACTOR_MAX_RANGE(pl->item[ITEM_TRACTOR_BEAM]))
    {
        double xvd, yvd, vel;
        int dir, away;
...
static bool Ball_handler(Player *pl)
{
    ...
    if (BIT(pl->have, HAS_BALL) || pl->ball)
    {
        ...
        dtdir = World_wrap_cfindDir(world,
            ...
            if (*mine_dist > (wire->wire_size == 1 ? 2 : 4) * BLOCK_SZ &&
                *mine_dist < 8 * BLOCK_SZ &&
                (delta_dir = (pl->dir -
                              World_wrap_cfindDir(
                                  world,
                                  shot->pos.cx - pl->pos.cx,
                                  shot->pos.cy - pl->pos.cy)) < wire->wire_size * (ANGLE_RESOLUTION / 10) ||
                             delta_dir > ANGLE_RESOLUTION - wire->wire_size * (ANGLE_RESOLUTION / 10)))
...
static void Robot_default_play_check_lasers(Player *pl)
{
    world_t *world = &World;
    int j;
    int dcx, dcy;
    ...
    if (BIT(pl->used, HAS_SHIELD) == 0 && BIT(pl->have, HAS_SHIELD) != 0)
    {
        ...
        for (j = 0; j < NumPulses; j++)
        {
            ...
            if (pl->id == pulse->id && options.selfImmunity)
                continue;
            dcx = WORLD_WRAP_DCX(world, pl->pos.cx - pulse->pos.cx);
            dcy = WORLD_WRAP_DCY(world, pl->pos.cy - pulse->pos.cy);
...
static void Robot_default_play(Player *pl)
{
    ...
    if (BIT(pl->lock.tagged, LOCK_PLAYER))
    {
        ...
        ship = Player_by_id(pl->lock.pl_id);
        delta_dir = (int)(pl->dir -
                          World_wrap_cfindDir(world,
                                              ship->pos.cx - pl->pos.cx,
                                              ship->pos.cy - pl->pos.cy));
