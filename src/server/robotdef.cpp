...
static bool Check_robot_evade(int ind, int mine_i, int ship_i)
{
    ...
    stop_dist =
        (long)((RES * velocity) / (MAX_PLAYER_TURNSPEED * pl->turnresistance) + (velocity * velocity * pl->mass) / (2 * MAX_PLAYER_POWER) + safe_width);
    /*
    ...
    px[0] = pl->pix_pos.x;                             /* ship center x */
    py[0] = pl->pix_pos.y;                             /* ship center y */
                               ...
                               int new_mode)
{
    ...
    dx = item_x - pl->pix_pos.x, dx = WRAP_DX(dx);
    dy = item_y - pl->pix_pos.y, dy = WRAP_DY(dy);
    ...
    for (dist = 0; clear_path && dist < item_dist; dist += BLOCK_SZ / 2)
    {
        ...
        dx = (long)((pl->pix_pos.x + dist * tcos(item_dir)) / BLOCK_SZ);
        dy = (long)((pl->pix_pos.y + dist * tsin(item_dir)) / BLOCK_SZ);
    ...
    if (new_mode == RM_ATTACK || (BIT(world->rules->mode, TIMING) && new_mode == RM_NAVIGATE))
    {
        ...
        {
            if (BIT(my_data->robot_lock, LOCK_PLAYER) && BIT(PlayersArray[GetIndArray[my_data->robot_lock_id]]->obj_status,
                                                             PLAYING | PAUSE | GAME_OVER) == PLAYING)
                ship = PlayersArray[GetIndArray[my_data->robot_lock_id]];
            else if (BIT(pl->lock.tagged, LOCK_PLAYER))
                ship = PlayersArray[GetIndArray[pl->lock.pl_id]];
            else
                ship = NULL;
        ...
        else if ((my_data->robot_count % 10) < pl->item[ITEM_MISSILE] && !WITHIN(my_data->robot_count, my_data->last_fired_missile, 10))
        {
            ...
            if (Detect_hunt(ind, GetIndArray[pl->lock.pl_id]) && !pl->visibility[GetIndArray[pl->lock.pl_id]].canSee)
                type = OBJ_HEAT_SHOT_BIT;
...
static bool Check_robot_hunt(int ind)
{
    ...
    if (pl->fuel.sum < pl->fuel.l3 /*MAX_PLAYER_FUEL/2*/)
        return false;
    if (!Detect_hunt(ind, GetIndArray[my_data->robot_lock_id]))
        return false;
...
static bool Detect_hunt(int ind, int j)
{
    ...
    if (BIT(ship->obj_status, PLAYING | PAUSE | GAME_OVER | KILLED) != PLAYING)
    {
...
static bool Ball_handler(int ind)
{
    ...
    if (BIT(pl->have, HAS_BALL) || pl->ball)
    {
        ...
        tdir = (int)Wrap_findDir((world->treasures[closest_t].blk_pos.x + 0.5) * BLOCK_SZ - ball->pix_pos.x,
                                 (world->treasures[closest_t].blk_pos.y + 0.5) * BLOCK_SZ - ball->pix_pos.y);
    ...
    else
    {
        ...
        for (i = 0; i < NumObjs; i++)
        {
            if (Obj[i]->type == OBJ_BALL_BIT)
            {
                ...
                if ((ball->id == NO_ID)
                        ? (ball->owner != NO_ID)
                        : (PlayersArray[GetIndArray[ball->id]]->team != pl->team))
                {
                    ball_dist = (int)LENGTH(pl->pix_pos.x - ball->pix_pos.x,
                                            pl->pix_pos.y - ball->pix_pos.y);
        ...
        if (closest_ball == -1 && closest_nt_dist < (my_data->robot_count / 10) * BLOCK_SZ)
        {
            SET_BIT(my_data->longterm_mode, FETCH_TREASURE);
            return (Check_robot_target(ind,
        ...
        {
            SET_BIT(my_data->longterm_mode, FETCH_TREASURE);
            return (Check_robot_target(ind, Obj[closest_ball]->pix_pos.x,
                                       Obj[closest_ball]->pix_pos.y,
                                       RM_NAVIGATE));
        }
    }
    return false;
...
static int Robot_default_play_check_map(int ind)
{
    ...
    for (j = 0; j < world->NumTargets; j++)
    {
        ...
        if ((dx = world->targets[j].blk_pos.x * BLOCK_SZ + BLOCK_SZ / 2 - pl->pix_pos.x,
             dx = WRAP_DX(dx), ABS(dx)) < target_dist &&
            (dy = world->targets[j].blk_pos.y * BLOCK_SZ + BLOCK_SZ / 2 - pl->pix_pos.y,
    ...
    if (fuel_i >= 0 && (target_dist > fuel_dist || !BIT(world->rules->mode, TEAM_PLAY)) && BIT(my_data->longterm_mode, NEED_FUEL))
    {
...
static void Robot_default_play(int ind)
{
    ...
    if (BIT(world->rules->mode, TEAM_PLAY))
    {
        for (j = 0; j < world->NumTargets; j++)
        {
            ...
            {
                int dx = (world->targets[j].blk_pos.x * BLOCK_SZ + BLOCK_SZ / 2) - pl->pix_pos.x;
                int dy = (world->targets[j].blk_pos.y * BLOCK_SZ + BLOCK_SZ / 2) - pl->pix_pos.y;
                /* dx = WRAP_DX(dx);
                   dy = WRAP_DY(dy); */
    ...
    /* KK: unfortunately, this introduced a new bug. robots with large
        ...
        fix it */
    if (QUICK_LENGTH(pl->pix_pos.x - (world->bases[pl->home_base].blk_pos.x * BLOCK_SZ),
                     pl->pix_pos.y - (world->bases[pl->home_base].blk_pos.y * BLOCK_SZ)) < BLOCK_SZ)
    {
        SET_BIT(pl->obj_status, THRUSTING);
    }
    ...
    if (BIT(my_data->robot_lock, LOCK_PLAYER))
    {
        j = GetIndArray[my_data->robot_lock_id];
        ship = PlayersArray[j];

        if (BIT(PlayersArray[GetIndArray[my_data->robot_lock_id]]->obj_status,
                PLAYING | GAME_OVER | PAUSE) == PLAYING)
        {
            ...
            if (Detect_hunt(ind, j))
            {
                ...
                {
                    ship_i = GetIndArray[my_data->robot_lock_id];
                    ship_dist = distance;
                }
    ...
    if (ship_i == -1 || enemy_i == -1)
    {
        ...
        {
            ship = PlayersArray[j];
            if (j == ind || BIT(ship->obj_status, PLAYING | GAME_OVER | PAUSE) != PLAYING || Team_immune(pl->id, ship->id))
                continue;
    ...
    if (enemy_i >= 0)
    {
        ...
        if (!BIT(pl->lock.tagged, LOCK_PLAYER) || (enemy_dist < pl->lock.distance / 2 && (BIT(world->rules->mode, TIMING) ? (ship->check >= pl->check && ship->round >= pl->round) : 1)) || (enemy_dist < pl->lock.distance * 2 && BIT(world->rules->mode, TEAM_PLAY) && BIT(ship->have, HAS_BALL)) || ship->score > PlayersArray[GetIndArray[pl->lock.pl_id]]->score)
        {
    ...
    if (BIT(pl->lock.tagged, LOCK_PLAYER))
    {
        ...
        if (BIT(ship->obj_status, PLAYING | PAUSE | GAME_OVER) != PLAYING || (BIT(my_data->robot_lock, LOCK_PLAYER) && my_data->robot_lock_id != pl->lock.pl_id && BIT(PlayersArray[GetIndArray[my_data->robot_lock_id]]->obj_status, PLAYING | PAUSE | GAME_OVER) == PLAYING) || !Detect_hunt(ind, GetIndArray[ship->id]) || (pl->fuel.sum <= pl->fuel.l3 && !BIT(world->rules->mode, TIMING)) || (BIT(world->rules->mode, TIMING) && (delta_dir < 3 * RES / 4 || delta_dir > RES / 4)) || Team_immune(pl->id, ship->id))
        {
    ...
    if (item_i >= 0 && 3 * enemy_dist > 2 * item_dist && item_dist < 12 * BLOCK_SZ && !BIT(my_data->longterm_mode, FETCH_TREASURE) && (!BIT(my_data->longterm_mode, NEED_FUEL) || Obj[item_i]->info == ITEM_FUEL || Obj[item_i]->info == ITEM_TANK))
    {
        ...
        if (item_imp != ROBOT_IGNORE_ITEM)
        {
            ...
            dx = Obj[item_i]->pix_pos.x;
            dx += (long)(Obj[item_i]->vel.x * (ABS(dx - pl->pix_pos.x) /
                                               my_data->robot_normal_speed));
            dy = Obj[item_i]->pix_pos.y;
            dy += (long)(Obj[item_i]->vel.y * (ABS(dy - pl->pix_pos.y) /
                                               my_data->robot_normal_speed));
    ...
    if (BIT(pl->lock.tagged, LOCK_PLAYER) &&
        Detect_hunt(ind, GetIndArray[pl->lock.pl_id]))
    {
        ...
        if (Check_robot_target(ind, dx, dy, RM_ATTACK) && !BIT(my_data->longterm_mode, FETCH_TREASURE | TARGET_KILL | NEED_FUEL))
        {
            return;
        }
    }
