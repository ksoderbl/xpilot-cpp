    ...
    for (i = 0; i < NumPlayers; i++)
    {
        ...
        if (BIT(world->rules->mode, CRASH_WITH_PLAYER | BOUNCE_WITH_PLAYER))
        {
            for (j = i + 1; j < NumPlayers; j++)
            {
                ...
                if (pl->fuel.oldSum <= 0 || (!BIT(pl->used, HAS_SHIELD) && !BIT(pl->have, HAS_ARMOR)))
                    SET_BIT(pl->obj_status, KILLED);

                if (pl_j->fuel.oldSum <= 0 || (!BIT(pl_j->used, HAS_SHIELD) && !BIT(pl_j->have, HAS_ARMOR)))
...
static void Player_collides_with_mine(player_t *pl, mineobject_t *mine)
{
    ...
    else
    {
        ...
        sprintf(msg, "%s hit %s %s by %s and reprogrammed by %s.",
                ...
                Describe_shot(mine->type, mine->obj_status, mine->mods, 1),
                BIT(mine->obj_status, GRAVITY) ? "thrown " : "dropped ",
                PlayersArray[GetInd(mine->mine_owner)]->name,
                reprogrammer_name);
    }
    if (kp)
    {
...
static void Player_collides_with_debris(player_t *pl, object_t *obj)
{
    ...
    if (pl->fuel.oldSum == 0 || (obj->type == OBJ_WRECKAGE_BIT && options.wreckageCollisionMayKill && !BIT(pl->used, HAS_SHIELD) && !BIT(pl->have, HAS_ARMOR)))
    {
        SET_BIT(pl->obj_status, KILLED);
...
static void Player_collides_with_asteroid(player_t *pl, wireobject_t *ast)
{
    ...
    if (options.asteroidCollisionMayKill && (pl->fuel.oldSum == 0 || (!BIT(pl->used, HAS_SHIELD) && !Player_has_armor(pl))))
    {
        int sc;
        SET_BIT(pl->obj_status, KILLED);
        if (pl->velocity > v)
...
static void Player_collides_with_killing_shot(player_t *pl, object_t *obj)
{
    ...
    if (BIT(pl->used, HAS_SHIELD) || BIT(pl->have, HAS_ARMOR) || (obj->type == OBJ_TORPEDO && BIT(obj->mods.nuclear, NUCLEAR) && (int)(rfrac() >= 0.25)))
    {
        switch (obj->type)
        {
        case OBJ_TORPEDO:
