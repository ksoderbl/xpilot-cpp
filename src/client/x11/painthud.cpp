...
static void Paint_lock(int hud_pos_x, int hud_pos_y)
{
    ...
    int x, y;
    int i, dir = 96;
    int hudShipColor = clientOptions.hudColor;
    Other *target;
    ShipShape *ship;
    char str[50];
    ...
    rd.drawString(dpy, drawPixmap, gameGC,
                  ...
                  target->nick_name.c_str(), target->name_len);
    ...
    if (lock_dist != 0)
    {
        if (lock_dist > WARNING_DISTANCE || (loopsSlow & 1))
        {
            ...
            if (self != nullptr && ((self->team == target->team && BIT(Setup->mode, TEAM_PLAY)) || (self->alliance != ' ' && self->alliance == target->alliance)))
                Arc_add(clientOptions.hudColor,
...
void Paint_hudradar(void)
{
    ...
    for (const auto &radarObject : clMap.radarObjects)
    {
        ...
        if ((sz & 0x80) == 0)
        {
            ...
            Arc_add(clientOptions.hudRadarEnemyColor,
...
void Paint_HUD(void)
{
    ...
    if (clientOptions.hudHLineColor > BLACK)
    {
    ...
    if (clientOptions.hudVLineColor > BLACK)
    {
    ...
    for (i = 0, j = 0; i < MAX_SCORE_OBJECTS; i++)
    {
        ...
        if (sobj->hud_msg_len > 0)
        {
            if (j == 0 &&
                ...
                (did_fuel || clientOptions.hudVLineColor > BLACK))
                ++j;
            rd.drawString(dpy, drawPixmap, gameGC,
