...
static char *Widget_strdup(const char *str)
{
    ...
    len = strlen(str) + 1;
    if ((copy = XMALLOC(char, len)) == nullptr)
        return nullptr;
                        ...
                        void *user_data)
{
    ...
    if ((parent_widget = Widget_pointer(parent_desc)) == nullptr || parent_widget->type != WIDGET_FORM)
    {
        warn("Widget_create_color: Invalid parent widget");
        return NO_WIDGET;
    }
    if ((colorw = XMALLOC(widget_color_t, 1)) == nullptr)
    {
        error("No memory for color widget");
        return NO_WIDGET;
    }
    colorw->val = val;
    colorw->min = min;
