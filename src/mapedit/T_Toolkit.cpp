...
void T_ConnectToServer(const char *display_name)
{
    ...
    if ((display = XOpenDisplay(display_name)) == nullptr)
    {
...
{
    if ((*fontinfo = XLoadQueryFont(display, fontname)) == nullptr)
    {
                  ...
                  const char *string, int justify, int crop, int cursorpos)
{
    ...
    XDrawString(display, win, gc, (int)(x + XTextWidth(T_Font, string, cursorpos) - XTextWidth(T_Font, CURSOR_CHAR, 1) / 2),
